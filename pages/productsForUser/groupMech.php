...
<?php
			...
			<?php
			$sql = mysql_query("SELECT product.idProduct, product.model, product.price, product.Image FROM product, ProductCategoryr WHERE product.idProduct = ProductCategoryr.idProduct AND ProductCategoryr.idCategory = 8", $link);
			...
			<?php
			$sql = mysql_query("SELECT product.idProduct, product.model, product.price, product.Image FROM product, ProductCategoryr WHERE product.idProduct = ProductCategoryr.idProduct AND ProductCategoryr.idCategory = 9", $link);
			while ($result = mysql_fetch_array($sql)) {
			//echo "$result[6]";
			echo     '<tr><td>'.$result[1].'</td>'.
			         '<td>'.$result[2].'</td>'.
